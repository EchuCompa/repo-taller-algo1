...
tuple<int,int> encontrar_valle_recur(int i, int j,vector<vector<int>>  terreno) {
    tuple<int,int> posicion= make_tuple(i,j);
    vector<tuple<int,int>> vecis_debajo = vecinosDebajo(posicion,terreno);
    for (int i=0;i < vecis_debajo.size();i++) {
        tuple<int,int> nueva_posi = encontrar_valle_recur(get<0>(vecis_debajo[i]), get<1>(vecis_debajo[i]), terreno);
        if (terreno[get<0>(nueva_posi)][get<1>(nueva_posi)] < terreno[get<0>(posicion)][get<1>(posicion)] ){
              posicion = nueva_posi;
            }
    }
    return posicion;
...
vector<tuple<int,int>> vecinosDebajo(tuple<int,int> posi,vector<vector<int>>  terreno) {
    ...
    for (int k =i-1; k<i+2;k++) {
        if ( k>=0 && k<terreno.size() ) { //Si k está en rango
            for (int l = j - 1; l < j + 2; l++) {
                if ( (j>=0 && k<terreno.size())  && (terreno[k][l] < terreno[i][j] && (k!=i || l!=j) ) ) {
...
/***
   v >  |      v       |   v        Cada ">" representa la dirección
...
bool hayGanadorEnDireccion(vector<vector<char>> tablero, tuple<int,int> posi, tuple<int,int> dire, bool& completo) {
    bool ganador= true;
    ...
    for (int k=0;k<2;k++) { //Reviso si ambas posiciones tienen el mismo caracter
        x += i;
        y += j;
        ganador = ganador && (tablero[x][y] == figu);
        completo = completo && tablero[x ][y] != ' '; //Esto es para revisar "de chapa"         // si en algún lugar del tablero hay una vacía
    }
...
//Además no es un tablero invalido si hay más "X" que "O" o viceversa, es inclusivo con cualquier TaTeTi.
//En otras palabras alta paja programar que cuente si |cant_x - cant_o | <= 1
...
void revisarTablero(vector<vector<char>> tablero) {
    ...
    for (int i=0;i<3;i++) {
        ...
        if (hayGanadorEnDireccion(tablero, make_tuple(i,0),make_tuple(0,1), completo) ) {
            lineas_ganadoras +=1;
            char_ganador = tablero[0][i];
        }
        //Reviso las lineas horizontales
        if (hayGanadorEnDireccion(tablero, make_tuple(0,i),make_tuple(1,0), completo) ) {
            lineas_ganadoras +=1;
            char_ganador = tablero[0][i];
        }
    ...
    if (!completo && lineas_ganadoras == 0){
        cout << "Juego en progreso. Este duelo de epicas proporciones no ha terminado.";
    }
}
...
bool nuevaReinaValida( vector<pair<int,int>> posiciones_reinas,int colu,int fila) {
    bool funca = true;
    for (int j = 0; j < posiciones_reinas.size() - 1; j++) { //Reviso que sea una reina válida
        int x = posiciones_reinas[j].first;
        int y = posiciones_reinas[j].second;
        if (x == colu || y == fila || abs(x - colu) == abs(y - fila)) {
            funca = false;
            break;
        }
    }
...
bool ubicarReinaEnIesimaColumna(vector<pair<int,int>>  posi_reinas, int colu, int n) {
    if (colu ==n) { //Si ya coloque todas las reinas devuelvo true
        cout << "[ ";
        for (pair<int,int> t : posi_reinas) { //Imprime la solución en la terminal
            cout << "( " << t.first << ", " << t.second << " ) ";
        }
        cout << " ] " << endl;
        //return true; //Aca meramente devuelvo la primera que encuentre
        return false; //Acá devuelvo todas las soluciones
    }
    vector<pair<int,int>>  nueva_posi_reinas;
    bool esValida=false;
    for (int fila=0;fila<n;fila++) { //Compruebo si hay alguna fila en la cuál puedo insertar
        nueva_posi_reinas = posi_reinas;
        nueva_posi_reinas.push_back(make_pair(colu,fila)); //Agrego la nueva reina
        esValida = nuevaReinaValida(nueva_posi_reinas,colu,fila);

        if (esValida) { //Si es una posición válida entonces busco en la siguiente columna
            if (ubicarReinaEnIesimaColumna(nueva_posi_reinas,colu+1,n)){
                return true;
            }
        }
    }
    return false;  //Si no hay ninguna configuración válida entonces devuelvo false y backtrackeo
}
